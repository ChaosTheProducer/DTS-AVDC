...
class GoalGaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, x_cond, task_embed, return_all_timesteps=False, save_tokens=False):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            if save_tokens:
                # save images
                self._save_step_images(img, save_dir="./experiments_images",
                        step_idx=self.num_timesteps - 1 - t, t_value=t,
                        tag="psample",save_steps=save_tokens)
    ...
    """
    Draft Sample Generation with DDIM
    """
    @torch.no_grad()
    def ddim_sample(self, img, shape, x_cond, task_embed, target_timestep, num_blocks, return_all_timesteps=False):
        ...
        if target_timestep is None:
            ...
            remaining_steps = (target_timestep // sampling_timesteps) + 1
            times = torch.linspace(-1, target_timestep, steps = remaining_steps + 1)
        ...
        for step_idx, (time_step, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')): 
            ...
            cuda_time += starter.elapsed_time(ender)
        
            
            tensor_mem = torch.cuda.memory_allocated()
            current_max_mem = torch.cuda.max_memory_allocated(device) / (1024 ** 2)


            print(f"[Step {step_idx}] CUDA time: {cuda_time:.3f} ms | Max Mem: {current_max_mem:.2f} MB | Tensor Mem: {tensor_mem:.2f} MB")
            ...
            if time_next < 0:
                ...
                print(f"[DDIM Debug] Initial img shape: {img.shape}")
                continue 
    ...
    """
    @torch.no_grad()
    def target_sample_2(self, t_list, draft_tokens, shape, x_cond, task_embed, target_timestep, return_all_timesteps=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        
        print("ttttttttttttlist", t_list)
        ...
        for round_idx, (time, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')): 
            ...
            t_batch = torch.clamp(t_batch - 2, min=0)
            if time_next < 0 and round_idx==5:
                print("now time_next < 0 and round_idx==4")
    ...
    def speculative_decoding(self, shape, x_cond, text, draft_model, target_timestep=100,  num_blocks = None, return_all_timesteps=False):
        ...
        while True:
            ...
            ret, draft_tokens, t_list, target_timestep = self.ddim_sample(
                start_img, shape, x_cond, text, current_timestep, num_blocks, return_all_timesteps=False
            ) 

            # Target sampling
            target_tokens = self.target_sample_2(
                t_list, draft_tokens, shape, x_cond, text, current_timestep, return_all_timesteps=False
            )

            # Compare tokens and get acceptance information
            is_all_accept, reject_index = self.compare_tokens(draft_tokens, target_tokens, sim_threshold)
            
            round += 1

            """Acceptance handling logic"""
            if is_all_accept:
                good_tokens.extend(target_tokens)

                print(f"[DEBUG] good_tokens count: {len(good_tokens)}")
                print(f"[DEBUG] target_tokens count: {len(target_tokens)}")
                ...
                if current_timestep >= 0:
                    good_tokens.append(target_tokens[-1]) 
                    print("Continue Draft Sampling")
                    continue
                
                if len(good_tokens) > 0:
                    final_result = good_tokens[-1] 
                    print(f"[DEBUG] Using last good_token:")
                else:
                    # If no good tokens, use ret (from last ddim_sample)
                    final_result = ret
                    print(f"[DEBUG] Using ret")
                ...
                # Only part of the tokens are accepted

                if reject_index > 0:    
                    # Discard rejected tokens, keep accepted ones
                    accepted_tokens = target_tokens[:reject_index]
                    print("acc_token:", len(accepted_tokens))
                    good_tokens.extend(accepted_tokens)
                    print("good_token:", len(good_tokens))
    ...
    def compare_tokens(self, 
        ...
        Compare draft_tokens[i] and target_tokens[i]
        """
        ...
        for i in range(len(target_tokens)):
            ...
            if avg_l2 > sim_threshold:  # If L2 distance exceeds threshold, reject
                print("reject here", i)
                is_all_accept = False
                reject_index = i
    ...
    def sample(self, x_cond, 
        ...
        # If draft model is used, run speculative decoding
        if draft_model is True:
            print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDraft")
            num_blocks = self.num_blocks
            start_time = time_module.time()
            print("num_blocks:", num_blocks)
            good_tokens, imgs = self.speculative_decoding(
                (batch_size, channels, image_size[0], image_size[1]), 
                x_cond, task_embed, draft_model, target_timestep, num_blocks,
                return_all_timesteps = return_all_timesteps,
            )
            end_time = time_module.time()
            elapsed = end_time - start_time 
            print(f"[Timing] speculative_decoding time: {elapsed:.2f} seconds")

            log_file="timing_log.json"
        ...
        end_time = time_module.time()
        elapsed = end_time - start_time
        log_file="baseline_timing_log.json"
        with open(log_file, "a") as f:
            f.write(json.dumps(elapsed) + "\n")
        
        return ret.clamp(min=0, max=1)
